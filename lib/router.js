...
function Router () {
  ...
  router.middleware = function (sock, cb) {
    ...
    debug('typeof socket.onevent? %s', type);
    if (type === 'function') {
      debug('is a function');
      if (sock.onevent.isRouter) {
        ...
        debug('sock.event.stack?', sock.onevent.stack);

        if (!sock.onevent.stack.next) {
          debug('does not have next function');
          sock.onevent.stack.next = function (sock, args) {
            if (!sock) throw new Error('we need the socket');
            if (!args) throw new Error('we need the args');
            var step = this.shift();
            if ('function' === typeof step) {
              debug('we on a step right');
              debug('step', typeof step, sock, args);
              step(sock, args)
            }
            else {
              debug('we have no more steps, calling emit on the socket');
              emit.apply(sock, args);
            }
          };
        }

        debug('adding this router\'s onRoute method to the stack');
        sock.onevent.stack.push(function (sock, args) { 
          debug('we are about to call the onRoute method in this step');
          router.onRoute(sock, args); 
        });
      ...
      debug('not a function setting the onEvent handler');
      sock.onevent = router.onEvent;
    }
    cb();
  };
  ...
  router.onRoute = function (sock, args) {
    ...
    debug('onRoute %s, %s', sock.id, args);

    var done = function (emit, args) {
      debug('done callled');
      if (!sock.onevent.stack || !sock.onevent.stack.next) {
        debug('we do not have a stack of router\'s so we are done');
        emit.apply(sock, args);
      }
      else {
        debug('we have additional routers in the stack');
        sock.onevent.stack.next(sock, args);
      }
    };
